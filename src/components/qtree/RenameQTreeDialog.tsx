...
interface RenameQTreeDialogProps {
  currentQuestion: string;
  onConfirm: (question: string) => void;
  onCancel: () => void;
}

export function RenameQTreeDialog({ currentQuestion, onConfirm, onCancel }: RenameQTreeDialogProps) {
  ...
  return (
    <div className="fixed inset-0 bg-black/30 flex items-center justify-center z-50">
      <form onSubmit={handleSubmit} className="bg-white rounded shadow p-4 w-72">
        <h3 className="font-semibold mb-2">Rename Q Tree</h3>
        <input
          type="text"
          value={question}
